...
The rows below are exactly the operations in `templiqx_application::CAPABILITY_CATALOG`:
...
`templiqx catalog` reports this shared catalog; it is introspection, not an additional application operation.
