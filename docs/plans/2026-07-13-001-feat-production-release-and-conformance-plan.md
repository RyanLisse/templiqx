...
- At the owner's request, hosted cold-clone automation is disabled to avoid recurring Docker/kind runner cost. Cold reproducibility remains available locally through `just fresh-clone`; tagged releases scan every immutable image after its registry push and before signing or GitHub release creation, using pinned, checksum-verified Grype.
