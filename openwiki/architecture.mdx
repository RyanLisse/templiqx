...
filesystem composition + CAS writes"]
    CLI["templiqx-cli"]
    MCP["templiqx-mcp"]
    HTTP["templiqx-http / templiqx-http-server
northbound /operations/v1"]
...
The CLI, MCP server, and HTTP transport are peers: three thin surfaces over one service. Adding a transport never changes product semantics.
...
- `crates/templiqx-cli/src/main.rs`, `crates/templiqx-mcp/src/lib.rs`, and `crates/templiqx-http/src/lib.rs` are surfaces over the same operations.
...
- `adapters/templiqx-runtime-langfuse` is the host-owned production runtime that runs a real chat completion and traces to Langfuse; it is never in default composition.
- `adapters/templiqx-docx-v5` handles explicit DOCX V5 compatibility for the CRM3 fixture, and the bounded render adapters (`templiqx-html-plain`, `templiqx-markdown`, `templiqx-rtf`, `templiqx-typst`, `templiqx-xlsx`, `templiqx-tabular`) are host-constructed, deterministic document renderers kept out of the portable core.
